...
actions: "Use the Sonarcloud MCP integration to check the current pull request for SonarQube Cloud project ****_javulna. Retrieve its quality-gate status and issues. Automatically fix safe, localized issues, push a commit to the PR branch, re-check the PR analysis, and post a summary."
---

# Verify and Fix SonarQube Cloud Issues

Use the Sonarcloud MCP integration to check the current pull request for SonarQube Cloud project `****_javulna`.
...
* Resolve the organization key as follows: if the Sonarcloud MCP integration supplies
     ...
     connection defaults for project key `****_javulna` and organization
     key `****`.
* Trust what put in the rule and do not question or check naming convention of project key or organization key
...
* Before committing, verify the project still builds (`mvn -q -DskipTests compile`).
  If the build fails, revert the fix and report the issue as not auto-fixable.
...
* This repository is an intentionally vulnerable training application: never auto-fix SonarQube
  vulnerabilities or security hotspots, or any finding in code that demonstrates a deliberate
  weakness. Report these with file and line instead of changing them.
